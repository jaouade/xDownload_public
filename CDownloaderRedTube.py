    ...
    def download(self):
        if self.conexao.get_status():
            for self.i in range(self.pg_inicial, self.pg_final):
                response = requests.get(str('https://www.redtube.com/?search=' + self.pesquisa + '&page=' + str(self.i)))
                ...
                for span in soup.find_all(class_='video_thumb_wrap'):  # video-box four-column video_block_wrapper é a classe de div que contem os links
                    ...
                    comando = 'youtube-dl --verbose ' + link + '' + ' --output \\' + self.pasta_saida + '\\%(title)s.%(ext)s'
                    os.system(comando)
        else:
            self.tent += 1
